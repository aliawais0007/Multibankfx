...
$(function() {
    ...
    let clang = document.documentElement.lang;
    
    if (clang === 'ms') {
        clang = 'mys';
    }
    urlPath = urlPath.replace('/' + clang, '');
    if (clang === 'en') {
        urlPath = urlPath.replace('/ph', '');
    }
    ...
    $('.lang-menu-dropdown a, .lang-mobile-menu a').each(function() {
        ...
        if (aTag.attr('href') === '/') {
            ...
            aTag.attr('href', urlPath);
        } else {
            aTag.attr('href', aTag.attr('href') + urlPath);
        }
    });
});
	...
	this.dropzoneInput.on('change', function(e) {
		...
		for (let i = 0; i < uploadedFiles.length; i++) {
			if (self.numFiles >= self.maxNumFiles) {
				...
				self.dropzoneLabel.text('You have reached maximum number of 5 files');
...
FileDropZone.prototype.clearAllFiles = function() {
	this.dropzoneLabel.text('Drop files here or click to upload.');
...
FileDropZone.prototype.refreshDropzone = function() {
	...
	if (this.files.length > 0) {
		this.dropzoneLabel.addClass('yield-files');
                    ...
                    if (r.hasClass('ajax-submission')) {
                        ...
                        if (dropzone) {
                            if (dropzone.files.length === 0) {
                                $('.for_' + form_id).remove();
                                let errorsHtml = '<div class="errors error1 for_' + form_id + '"><div>Supporting Documents Required</div></div>';
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                try {
                                    ...
                                    $('.for_' + form_id).remove();
                                    let errorsHtml = '<div>Unknown Error</div>';
                            ...
                            error: function(xhr, textStatus, errorThrown) {
                                ...
                                if (xhr.status === 400) {
                                    try {
                                        ...
                                        errorsHtml = '<div>Unknown Error</div>';
                                    }
                                } else {
                                    errorsHtml = '<div>Unknown Error</div>';
                                }
    ...
    switch (e) {
        case "1":
            if (document.documentElement.lang === 'ae' || document.documentElement.lang === 'ar') {
                $(".la-label-name").html('اسم الشركه<sup class="sup-size">*</sup>');
            } else {
        ...
        case "3":
            if (document.documentElement.lang === 'ae' || document.documentElement.lang === 'ar') {
                $(".la-label-name").html('الاسم الكامل لصحاب الحساب الاول\n<sup class="sup-size">*</sup>');
            } else {
        ...
        default:
            if (document.documentElement.lang === 'ae' || document.documentElement.lang === 'ar') {
                $(".la-label-name").html('الاسم الكامل<sup class="sup-size">*</sup>')
            } else {
                    ...
                    renderBullet: function (e, a) {
                        switch (e) {
                            ...
                            case 3:
                                ...
                                break;
                            case 4:
                                (text = "DFSA"), (img_url = "/public_files/images/common/country/uae.png");
                                break;
                            case 5:
